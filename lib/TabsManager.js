...
class TabsManager
{
    ...
    static setup(mainWindow, mainTab)
    {
        ...
        TabsManager.TabLog("setup main configuration done");
    }
    ...
    static newTab(newTab, tabId = 'tab', update_modules = true, bounds = TabsManager.default_bounds)
    {
        newTab.preventDefault = (ext_name) => {newTab.preventCreation = true; newTab._ext_name = ext_name}
        if (update_modules)
        {
            // Watch out! This emitter waits for any extension's opinion, but only if they are not async!
            TabsManager.mainWindow.emit('before-new-tab-created', newTab);
            if (newTab.preventCreation)
            {
                TabsManager.TabLog(newTab._ext_name ? newTab._ext_name : "an extension", 'prevented this tab\'s creation');
                return this.closeTab(newTab);
            }
        }
        ...
        if (update_modules)
            ...
            TabsManager.mainWindow.emit('new-tab-created', newTab);
    }
    ...
    static setNewTab(newTab, tabId = 'tab', update_modules = true, bounds)
    {
    ...
    {
        TabsManager.emit('tab-closed', TabsManager.tabs["tab_" + tabId]);
