...
class TabsManager
{
    ...
    static resetTabSize()
    {
        TabsManager.log("Resizing all tabs");
        if (TabsManager.activeTab.tab)
            TabsManager.resized(TabsManager.activeTab);
        Object.values(TabsManager.sideTabs).forEach(element => {
            TabsManager.resized(element);
        });
    }

    static resized(tabwrap)
    {
        ...
        // TabsManager.log(tabwrap);
    }

    static newTab(newTab, tabname = 'tab', update_modules = true, bounds = TabsManager.default_bounds)
    {
        if (TabsManager.tabs[tabname]) throw new TabsManager.TabError('duplicate tab name: ' + tabname);
        if (update_modules)
        {
    ...
    static setTab(tabname, do_focus = false)
    {
    ...
    {
        const lastTabName = TabsManager.lastActiveTabName;
        TabsManager.log("Switching back to", lastTabName);
        if (TabsManager.activeTab)
            TabsManager.mainWindow.contentView.removeChildView(TabsManager.activeTab.tab);
        TabsManager.setTab(setThisTab || TabsManager.lastActiveTabName || TabsManager.defaultTabName);
    }

    static setNewTab(newTab, tabname = 'tab', update_modules = true, bounds = undefined, do_focus = false)
    {
        return TabsManager.newTab(newTab, tabname, update_modules, bounds) && TabsManager.setTab(tabname, do_focus);
    }

    static getTabName(find_tab)
    {
        return find_tab.tabname;     // just in case I forgor
    }
    ...
    {
        if (!tab?.tab_id || !TabsManager.tabs[tab.tab_id]) return this.err(tab, 'is not a valid closable tab!');
    ...
    static TabError = class extends TypeError {name = 'TabError'}
}
