...
// looks within the args given to the executable
function findAppArg(arg) {
	if (app.args)
		return app.args.find(function (v) {
			return v == arg;
		});
	return;
}

// looks for the page's title as soon as possible
...
// looks for favico in the various subdomains of the given url
async function getFavIco(domain) {
	...
	return ''; // Ultimate fallback (rendered sets to empty)
}
