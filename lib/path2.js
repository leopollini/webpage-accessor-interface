...
class path2
{
    ...
    // returns location inside extensions_data in app's root (inside appimage: sandbox!!!) -- READONLY
...
if (!path2.already_required && app && !Env.IS_EXECUTABLE)
{
    app.whenReady().then(function ()
    {
        ...
        {
            try {console.log('creating dir',path2.config_location );
            fs.symlinkSync(path2.config_location, path.join(__dirname, '..', EXT_CONFIGS_DIR));}
            catch (e) {console.log("Error linking dirs");}
        }
    });
