...
class BaseModule
{
	...
	__start(window, tab, data)
	{
		...
		if (!this.module_name || this.module_name == '')
			throw new BaseModule.ModuleError('Module name not specified');
    ...
    log(...message)
	{
		if (Env.VERBOSE) console.log('[', (this.HIGHLIGHT ? kleur.bold().green : kleur.green)(this.module_name), ']:', ...message);
	}
    warn(...message)
	{
		if (Env.VERBOSE) console.log('[', (this.HIGHLIGHT ? kleur.bold().yellow : kleur.yellow)(this.module_name), ']:', ...message);
	}
    err(...message)
	{
		if (Env.VERBOSE) console.log('[', (this.HIGHLIGHT ? kleur.red().green : kleur.red)(this.module_name), ']:', ...message);
	}
	...
	newCtrlShortcut(key, callback, overload_tab, do_prevent_default = true)
	{
		...
		tab.webContents.on('before-input-event', (event, input) => {
			if (!BaseModule.shorcurtLock && input.control && input.key.toLowerCase() === key)
			{
				...
				callback(tab);
				BaseModule.shorcurtLock = setTimeout(() => {BaseModule.shorcurtLock = undefined}, 150);
			}
		});
	}

	static ModuleError = class extends Error {name = "ModuleError"};
	static LoadError = class extends BaseModule.ModuleError {name = "LoadError"};
	// setup Windows??
}
