...
class BaseModule {
	...
	__start(window, tab, data) {
		...
		if (fs.existsSync(this.conf_file_path))
			...
			if (!this.FORCE_RUN) {
				this.silent(`Config file at ${this.conf_file_path} does not exist. Possibly disabled in config.json`);
	...
	silent(...message) {
		BaseModule.loggers.forEach((logger) => logger?.silent(this.module_name, message, this.HIGHLIGHT));
	}
	log(...message) {
