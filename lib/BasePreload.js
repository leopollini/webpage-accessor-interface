...
class BasePreload
{
	...
	async __start()
	{
		...
		if (fs.existsSync(this.conf_file_path))
			...
			catch (e) { throw new BasePreload.PreloadError("Could not load config file (" + e + ').'); }
		else
		{
	...
	static PreloadError = class extends require('./BaseModule').ModuleError {name = "PreloadError"};
