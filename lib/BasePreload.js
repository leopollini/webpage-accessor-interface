...
const { PreloadError } = require('./BasePreload');
...
class BasePreload {
	...
	async __start() {
		...
		if (this.contextbridge_expose) ipcChannel.exposeContextBridge(this.contextbridge_expose);

		// the setup calls are managed IN PARALLEL. NO SORTING ORDER IS GRANTED FOR PRELOADED SCRIPTS.

		if (Env.DEBUG_MODE) console.log('loading', kleur.green(this.module_name));
		...
		if (fs.existsSync(this.conf_file_path))
			try {
				...
				throw new BasePreload.PreloadError('Could not load config file (' + e + ').');
			}
		else {
			throw new BasePreload.PreloadError('Config file not created: containing module disabled in config');
		}
	...
	log(...message) {
		if (Env.VERBOSE) console.log('[ \x1b[32m%s\x1b[0m', this.module_name, ']:', ...message);
	}
	warn(...message) {
		if (Env.VERBOSE) console.log('[ \x1b[33m%s\x1b[0m', this.module_name, ']!:', ...message);
	}
	err(...message) {
		if (Env.VERBOSE) console.log('[ \x1b[31m%s\x1b[0m', this.module_name, ']!!:', ...message);
	}
	...
	static PreloadError = class extends require('./BaseModule').ModuleError {
		name = 'PreloadError';
	};
